defmodule Protohackers.Mitm.Connection do
  ...
  def handle_info({:tcp, incoming_socket, data}, %__MODULE__{incoming_socket: incoming_socket} = state) do
    :ok = :inet.setopts(incoming_socket, active: :once)
    Logger.debug("Received data from incoming socket: #{inspect(data)}")
    # Send data to outgoing_socket.
    :gen_tcp.send(state.outgoing_socket, data)
    {:noreply, state}
  end

  def handle_info({:tcp, outgoing_socket, data}, %__MODULE__{outgoing_socket: outgoing_socket} = state) do
    :ok = :inet.setopts(outgoing_socket, active: :once)
    Logger.debug("Received data from outgoing socket: #{inspect(data)}")
    # Send data to incoming_socket.
